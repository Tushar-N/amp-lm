...
cmd:option('-test_file', '../clustal_pruning/distinct_seqs.txt')
...
local f_pred=io.open('data/output_preds.txt','w')
local line_no=1
for seq in io.lines(opt.test_file) do
	--local sa_score, seq=unpack(line:split(' '))

	if seq:find('X')==nil and seq:find('B')==nil and seq:find('U')==nil then --ignore strange sequences
		pred = get_pred(seq)[1][1]
		f_pred:write(seq..'\t'..string.format('%.6f', pred)..'\n')

		if line_no%100==0 then
			print (line_no, seq, pred)
		end
